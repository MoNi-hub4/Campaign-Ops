...
export default function Workstreams({
  ...
  const handleOpenEdit = (tmpl, e) => {
    ...
    setFormData({
      ...
      sub_tasks:
        Array.isArray(tmpl.sub_tasks) && tmpl.sub_tasks.length > 0
          ? [...tmpl.sub_tasks]
          : [""],
    });
    setIsModalOpen(true);
  ...
  const handleFormSubmit = async (e) => {
    ...
    const endpoint = editingId
      ? `/api/workstreams/templates/${encodeURIComponent(editingId)}`
      : "/api/workstreams/templates";
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      const res = await fetch(
        `/api/workstreams/templates/${encodeURIComponent(id)}`,
        {
          method: "DELETE",
        },
      );
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className="p-4 sm:p-8 max-w-7xl">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400 font-medium">
              No templates found. Click "Add Workstream" to create one.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 items-start">
              {templates.map((tmpl) => {
                const subTaskCount = Array.isArray(tmpl.sub_tasks)
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-purple-700 bg-purple-50 px-2.5 py-1 rounded-full border border-purple-100">
                            {subTaskCount}{" "}
                            {subTaskCount === 1 ? "Sub-task" : "Sub-tasks"}
                          </span>
                      ...
                      {isExpanded && subTaskCount > 0 && (
                        <div className="mt-4 pt-3 border-t border-gray-100 space-y-2">
                          ...
                          {tmpl.sub_tasks.map((subTitle, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-2 text-xs text-gray-700 font-medium bg-gray-50/80 px-3 py-2 rounded-xl border border-gray-100"
                            >
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-gray-100 max-h-[90vh] flex flex-col">
            <div className="bg-gray-900 text-white p-6 flex justify-between items-center shrink-0">
              <h3 className="text-lg font-bold">
                {editingId
                  ? "Edit Workstream Template"
                  : "New Workstream Template"}
              </h3>
              <button
