...
export default function Dashboard({ selectedRegion, setSelectedRegion, selectedMonth, setSelectedMonth }) {
  ...
  const handleSelectCampaign = (campaign) => {
    ...
    if (campaign && detailWindowRef.current) {
      detailWindowRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
  };
    ...
    fetch(`http://localhost:5000/api/coupons?region=${encodeURIComponent(selectedRegion)}`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          setPlannedCouponsCount(data.filter((c) => c.status === 'PLANNED').length);
          setExpiredCouponsCount(data.filter((c) => c.status === 'EXPIRED').length);
        }
        setLoading(false);
      })
      .catch((err) => {
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className="p-4 sm:p-8 max-w-7xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-1">
            <h2 className="text-xl font-bold text-gray-900">{selectedMonth} at a glance</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-xs hover:shadow-md transition-shadow">
              ...
              <div className="text-sm text-gray-500">2 live <span className="mx-0.5">•</span> 2 planning <span className="mx-0.5">•</span> 1 done</div>
            </div>
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <span className="text-sm font-medium text-gray-500 group-hover:text-gray-900 transition-colors">Coupons</span>
              ...
              <div className="text-sm text-gray-500">
                {loading ? (
                  ...
                  <>
                    <span className="font-medium text-green-600">{liveCouponsCount} live</span>
                    <span className="mx-1">•</span>
                    <span className="font-medium text-amber-600">{plannedCouponsCount} planned</span>
                    <span className="mx-1">•</span>
                    <span className="font-medium text-gray-400">{expiredCouponsCount} expired</span>
                  </>
                )}
              </div>
            </Link>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-xs hover:shadow-md transition-shadow">
              ...
              <div className="text-sm text-gray-500">8 campaign <span className="mx-0.5">•</span> 3 BAU</div>
            </div>

            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-xs hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <span className="text-sm font-medium text-gray-500">Execution complete</span>
                <CheckCircle2 className="w-5 h-5 text-gray-400 stroke-[1.5]" />
              </div>
              <div className="text-4xl font-bold text-gray-900 mb-2">79%</div>
              <div className="text-sm text-gray-500">14 of 18 workstreams done</div>
            </div>
          </div>
