...
import { Plus, X, Check, Sparkles, Calendar, Pencil, Trash2 } from 'lucide-react';

const warmThemeStyles = [
  {
    defaultBorder: 'border-amber-200 hover:border-amber-400 hover:shadow-amber-100/50',
    accentLine: 'bg-amber-400',
    progressFill: 'bg-amber-500',
  },
  {
    defaultBorder: 'border-orange-200 hover:border-orange-400 hover:shadow-orange-100/50',
    accentLine: 'bg-orange-400',
    progressFill: 'bg-orange-500',
  },
  {
    defaultBorder: 'border-rose-200 hover:border-rose-400 hover:shadow-rose-100/50',
    accentLine: 'bg-rose-400',
    progressFill: 'bg-rose-500',
  },
  {
    defaultBorder: 'border-yellow-200 hover:border-yellow-400 hover:shadow-yellow-100/50',
...
export default function CampaignWorkspaces({ 
  ...
  const handleDeleteCampaign = async (campaignId, e) => {
    if (e) e.stopPropagation();
    if (!window.confirm('Delete this campaign workspace and all its tasks?')) return;

    try {
      const res = await fetch(`http://localhost:5000/api/campaigns/${encodeURIComponent(campaignId)}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const formattedDateRange = formData.startDate && formData.endDate 
      ? formatDateRange(formData.startDate, formData.endDate) 
      : undefined;

    const endpoint = editingCampaignId
      ? `http://localhost:5000/api/campaigns/${encodeURIComponent(editingCampaignId)}`
  ...
  const filteredCampaigns = campaigns.filter((c) => {
    ...
    if (selectedRegion === 'SA Only') return c.markets.includes('KSA') || c.markets.includes('SA');
    return true;
  });
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-200/80 shadow-xs overflow-hidden mt-6">
      <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-white">
        <div>
          <h3 className="text-lg font-bold text-gray-950 tracking-tight">Campaign workspaces</h3>
          <p className="text-xs text-gray-400 font-medium">Manage active campaign execution checklists</p>
        </div>
      ...
      <div className="p-6">
        {loading ? (
          <div className="text-center py-12 text-gray-400 font-medium text-xs">Loading campaign workspaces...</div>
        ) : filteredCampaigns.length === 0 ? (
          <div className="text-center py-12 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200 p-8">
            <Sparkles className="w-8 h-8 text-amber-400 mx-auto mb-2" />
            <h4 className="text-sm font-bold text-gray-900">No campaigns found</h4>
            <p className="text-xs text-gray-400 mt-1 mb-4">Click "Add Campaign" to configure a new workspace and select workstreams.</p>
            <button
              onClick={handleOpenCreateModal}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredCampaigns.map((item, index) => {
              ...
              const progressPercent = item.totalTasks > 0 ? Math.round((item.completedTasks / item.totalTasks) * 100) : 0;
              const colorTheme = warmThemeStyles[index % warmThemeStyles.length];
              ...
              return (
                ...
                >
                  <div className={`absolute top-0 left-0 right-0 h-1 ${colorTheme.accentLine}`} />
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-1.5 pt-1">
                      ...
                      <div className="flex items-center gap-1.5 ml-2 shrink-0">
                        ...
                        <span className={`text-[11px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${getStatusBadge(item.status)}`}>
                          {item.status}
                        </span>
                      </div>
                    </div>
                    ...
                    <p className="text-xs font-semibold text-gray-500 mb-4">
                      {item.dateRange || 'Dates pending'} <span className="mx-1.5 text-gray-300">•</span> <span className="text-gray-700">{item.markets}</span>
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-center text-xs font-semibold text-gray-600">
                      <span>{item.completedTasks} / {item.totalTasks} tasks complete</span>
                      <span className="text-gray-500 font-bold">{item.proofsCount || 0} proofs</span>
                    </div>
                  </div>
                </div>
              );
        ...
        >
          ...
          >
            <div className="bg-gray-900 text-white p-6 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-lg font-bold">{editingCampaignId ? 'Edit Campaign Details' : 'New Campaign Entry'}</h3>
                <p className="text-xs text-gray-400 mt-0.5">
                  {editingCampaignId ? 'Update campaign name and metadata' : 'Define campaign info and select workstream templates'}
                </p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-1 rounded-full hover:bg-white/10 text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4 text-sm overflow-y-auto flex-1">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-gray-50/80 p-3 rounded-2xl border border-gray-200/80">
                  <div>
                    <span className="block text-[10px] font-extrabold text-gray-400 uppercase mb-1">Start Date</span>
                    <input
                      type="date"
                  ...
                  <div>
                    <span className="block text-[10px] font-extrabold text-gray-400 uppercase mb-1">End Date</span>
                    <input
                      type="date"
                ...
                {formData.startDate && formData.endDate && (
                  <span className="text-[11px] font-bold text-amber-600 block mt-1.5 ml-1">
                    Formatted Range: {formatDateRange(formData.startDate, formData.endDate)}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Target Markets</label>
                  <select
              ...
              {!editingCampaignId && (
                <div className="pt-2">
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-2">
                    Include Workstream Templates ({selectedTemplateIds.length} selected)
                  </label>

                  {availableTemplates.length === 0 ? (
                    <div className="text-xs text-gray-400 italic">No global templates found in database.</div>
                  ) : (
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {availableTemplates.map((tmpl) => {
                        const isChecked = selectedTemplateIds.includes(tmpl._id);
                        const subTaskCount = tmpl.sub_tasks ? tmpl.sub_tasks.length : 0;
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className={`w-5 h-5 rounded-md flex items-center justify-center border ${
                                isChecked ? 'bg-purple-700 border-purple-700 text-white' : 'border-gray-300'
                              }`}>
                                {isChecked && <Check className="w-3.5 h-3.5 stroke-[3]" />}
                              </div>
                              <div>
                                <h5 className="text-xs font-bold">{tmpl.name}</h5>
                                <p className="text-[11px] text-gray-500">{subTaskCount} process sub-tasks</p>
                              </div>
                            </div>
